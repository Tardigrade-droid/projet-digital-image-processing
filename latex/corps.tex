...
Pour les visages, nous utilisons la cascade de Haar d'OpenCV, le fichier \texttt{haarcascade\_frontalface\_default.xml}. OpenCV l'applique avec \texttt{CascadeClassifier}. Nous ne réentraînons pas ce fichier. La cascade compare des rectangles clairs et sombres sur une image intégrale, puis enchaîne ces tests du plus rapide au plus exigeant~\cite{viola}. Elle ne calcule pas une transformée en ondelettes.
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.92\linewidth]{figures/connexion.png}
\caption{Page de connexion de l'application publiée.}
\label{fig:connexion}
\end{figure}

\section{Mise en production}

Le conteneur Docker installe OpenCV sans interface graphique, copie le programme et les pages, et écrit ses données dans \texttt{DATA\_DIR}. Les fichiers \texttt{render.yaml} et \texttt{railway.toml} décrivent le même conteneur, un contrôle de santé sur \texttt{/api/health}, et l'emplacement du disque. Le secret de signature des cookies est une variable d'environnement, ou un fichier créé une fois sur le disque. Le changer déconnecte tous les comptes, ce qui est le comportement attendu d'une rotation de clé. L'instance en service est accessible à l'adresse \url{https://cctv-6n8v.onrender.com/}. Le code source est publié dans le dépôt \url{https://github.com/Tardigrade-droid/projet-digital-image-processing}.

Une offre qui s'endort et qui ne monte pas de disque ne convient pas : elle perdrait les comptes au premier arrêt. Ce dépôt exclut les photographies, la base des comptes, le secret et le document du cours.
...
La chaîne a été exécutée sur une séquence synthétique, afin de contrôler l'alerte de mouvement sans dépendre d'une webcam, puis dans le navigateur avec la démonstration intégrée. L'état affiché, le compteur de présence et l'apparition d'une photographie dans la liste ont été observés ensemble. Une adresse de flux refusée par le schéma, et une adresse injoignable, produisent un message dans l'interface sans arrêter le serveur. La figure~\ref{fig:alertes} reprend trois photographies écrites par le journal lors de ces essais : un mouvement confirmé après douze images, une alerte de visage, et un déplacement de caméra.

\begin{figure}[ht]
\centering
\begin{minipage}[t]{0.32\linewidth}
  \centering
  \includegraphics[width=\linewidth]{figures/mouvement.jpg}\\[0.35em]
  {\small Mouvement}
\end{minipage}\hfill
\begin{minipage}[t]{0.32\linewidth}
  \centering
  \includegraphics[width=\linewidth]{figures/visage.jpg}\\[0.35em]
  {\small Visage}
\end{minipage}\hfill
\begin{minipage}[t]{0.32\linewidth}
  \centering
  \includegraphics[width=\linewidth]{figures/deplacement.jpg}\\[0.35em]
  {\small Caméra déplacée}
\end{minipage}
\caption{Photographies d'alerte produites par l'application. Les cadres orange entourent le mouvement. Sur l'alerte de visage, le cadre rouge s'est posé sur la fenêtre en contre-jour. Le déplacement de caméra ne déclare aucun objet.}
\label{fig:alertes}
\end{figure}
...
La cascade frontale rate les profils, les visages trop petits et les visages masqués. Elle peut aussi accepter un contour rectangulaire très contrasté. Lors de l'essai à contre-jour de la figure~\ref{fig:alertes}, le rectangle s'est posé sur la fenêtre plutôt que sur le visage. Augmenter le nombre de voisins réduirait cette erreur et en introduirait d'autres sur les vrais visages éloignés. Le système ne doit donc pas être présenté comme un contrôle d'identité.
...
L'application web transforme cette chaîne en un outil. L'utilisateur voit les images intermédiaires, dessine les zones, et retrouve ensuite les photographies de son compte. Elle est publiée à l'adresse \url{https://cctv-6n8v.onrender.com/}. Le code source se trouve à l'adresse \url{https://github.com/Tardigrade-droid/projet-digital-image-processing}. La conception sépare le traitement, la session et le stockage, ce que les diagrammes de classes, de séquence et de déploiement rendent explicite.
